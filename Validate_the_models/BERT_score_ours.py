...
file2_path = "/data/home/luyizhuo/Datastation_lyz/Datasets/NSD_complete/Visual_instruct_tuning_data/recaptioned_data/reformat_detail_groundtruth.json"

# 结果文件所在的目录
results_dir = "/data/home/luyizhuo/Datastation_lyz/Models/UniBrain/results/train_stage1_2/reformated/"
...
refs2 = extract_sentences_from_json(file2_path, is_list_value=False)
...
for sub in subjects:
    ...
    # 动态构建当前被试的结果文件名
    file_name = f"{sub}_step_24000_prompts_MM_reformat.json"
    ...
    # b. 计算并汇报 vs COCO gt 的分数
    (P, R, F), _ = score(captions, refs2, lang="en", return_hash=True)

    p_mean, r_mean, f_mean = P.mean().item(), R.mean().item(), F.mean().item()
    print(f"  vs COCO gt: P={p_mean:.6f} R={r_mean:.6f} F={f_mean:.6f}")

    # 存储分数用于后续平均
    all_scores_coco['P'].append(p_mean)
    all_scores_coco['R'].append(r_mean)
    all_scores_coco['F'].append(f_mean)
...
if num_subjects_processed > 0:
    ...
    # 计算 COCO gt 的平均分
    avg_P_coco = np.mean(all_scores_coco['P'])
    avg_R_coco = np.mean(all_scores_coco['R'])
    avg_F_coco = np.mean(all_scores_coco['F'])
    print(f"\n平均分 (vs COCO gt) -> {hashname_model}:")
    print(f"  P={avg_P_coco:.6f} R={avg_R_coco:.6f} F={avg_F_coco:.6f}")
else:
