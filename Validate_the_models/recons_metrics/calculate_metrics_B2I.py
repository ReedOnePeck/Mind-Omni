...
img_indices_path = f'/data/home/luyizhuo/Datastation_lyz/Datasets/NSD_complete/NSD_fMRI_MNI_multi/test_data_sub{subj_ID}/test_img_index_start_from0.npy'
images_dir = '/data/home/luyizhuo/Datastation_lyz/Datasets/NSD_complete/NSD_imgs'

test_img_indices = np.load(img_indices_path)
total_samples = len(test_img_indices)


gt_imgs = np.zeros((total_samples, 3, 425, 425), dtype=np.float32)

missing_count = 0
missing_indices = []

for i in tqdm(range(total_samples), desc="读取图像进度"):
    num = test_img_indices[i]
    img_filename = f"{num:05d}.png"
    ...
    with Image.open(img_path) as img:
        ...
        # 调整图像尺寸为425x425（如果原始尺寸不同）
        if img.size != (425, 425):
            img = img.resize((425, 425), Image.Resampling.LANCZOS)

        # 转换为numpy数组并归一化到[0, 1]
        img_array = np.array(img, dtype=np.float32) / 255.0

        # 转换维度 (425, 425, 3) -> (3, 425, 425)
...
recons_folder = '/data/home/luyizhuo/Datastation_lyz/Models/UniBrain/results/image_reconstruction/VD/ours_pure_long_t2i/sub7/'
result = np.zeros((1000, 512, 512, 3), dtype=np.float32)

for i in tqdm(range(1000), desc="读取重建图像"):
    img_filename = f"{i:04d}.png"
    img_path = os.path.join(recons_folder, img_filename)
    with Image.open(img_path) as img:
        # 确保图像为RGB模式（处理可能的灰度图或RGBA图）
        ...
        # 转换为numpy数组并归一化到[0, 1]（可选，根据需求决定是否保留）
        img_array = np.array(img, dtype=np.float32) / 255.0
...
all_brain_recons = torch.tensor(result).to(device).to(all_images.dtype).clamp(0,1)
