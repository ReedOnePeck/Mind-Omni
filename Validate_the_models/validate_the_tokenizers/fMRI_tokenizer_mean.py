...
def identify_and_average(data):
    """
    识别形状为64的维度并在该维度上取平均

    Args:
        data: 形状为(B, 16, 64)的张量或数组

    Returns:
        在64维度上平均后的数据，形状为(B, 16)
    """
    ...
    if dim_64 is None:
        raise ValueError("数据中没有找到大小为64的维度")

    print(f"找到大小为64的维度: 维度{dim_64}")

    # 在64维度上取平均
    if isinstance(data, torch.Tensor):
        averaged_data = torch.mean(data, dim=dim_64)
    else:  # 假设是numpy数组
        averaged_data = np.mean(data, axis=dim_64)

    print(f"平均后数据形状: {averaged_data.shape}")
...
class ModelArgs:
    ...
    desired_token_num: int = 32 # 默认值为 64，保持与您现有行为一致
...
class VQ_fMRI(nn.Module):
    def __init__(self, config: ModelArgs):
        ...
        self.fmri_to_clip_proj = CLIPProjector(16, clip_embed_dim=1024)
    ...
    def forward(self,
        ...
        fmri_tokens_flattened = identify_and_average(fmri_tokens)
...
if __name__ == '__main__':
    ...
    a, b = fMRI_quantizer.forward_for_inference(fmri_data)
    quantized_fmri_tokens, codebook_indices = fMRI_quantizer(fmri_data, img_clip_feature, text_clip_feature,text_clip_hidden_features, text_input_ids, text_padding_mask)


    recon_fmri, losses = fMRI_quantizer.compute_losses(fmri_data, img_clip_feature, text_clip_feature,
                                                       text_clip_hidden_features, text_input_ids, text_padding_mask)
    print('')
