...
from MindOmni_src_stage2.r_tri_modal_attention_precessor import Trimodal_Attention, Trimodal_FluxAttnProcessor2_0
...
class FeedForward(nn.Module):
    r"""
    一个对 PEFT/LoRA 友好的前馈网络层 (FFN) 的重写版本。

    该版本经过严格设计，确保在 activation_fn="gelu-approximate" 的情况下，
    其计算图与原始版本完全等价。

    它将内部的两个线性层暴露为顶层属性 `proj_in` 和 `proj_out`，
    以便 LoRA 可以通过 target_modules=["proj_in", "proj_out"] 精确地定位它们。
    """
    ...
    def __init__(
            ...
            mult: int = 4,
            dropout: float = 0.0,
            activation_fn: str = "gelu-approximate",  # 此版本专注于此情况
        ...
        if activation_fn != "gelu-approximate":
            raise ValueError(
                f"此重写版本仅为 'gelu-approximate' 设计，但收到了 '{activation_fn}'"
            )
        ...
        # --- 核心改动：将原始 self.net 列表中的模块暴露为直接属性 ---

        # 1. 暴露第一个线性层 (原先封装在 GELU 模块内部)
        # 这个定义与原始 GELU 类内部的 self.proj 完全一致
        self.FFN_proj_in = nn.Linear(dim, inner_dim, bias=bias)

        # 2. 暴露激活函数
        # 我们将原始 GELU 类的 gelu 方法逻辑直接在此处实现
        self.activation = nn.GELU(approximate="tanh")

        # 3. 暴露第一个 Dropout 层 (原先是 self.net[1])
        self.dropout = nn.Dropout(dropout)

        # 4. 暴露第二个线性层 (原先是 self.net[2])
        self.FFN_proj_out = nn.Linear(inner_dim, dim_out, bias=bias)

        # 5. 暴露可选的 final_dropout 层 (原先是 self.net[3])
        if final_dropout:
            self.final_dropout = nn.Dropout(dropout)
        else:
            # 使用 Identity 层，避免在 forward 中写 if-else，保持计算图干净
    ...
    def forward(self, hidden_states: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        ...
        # --- 严格按照原始 self.net 列表的顺序执行计算 ---
        # 原始流程: GELU -> Dropout -> Linear -> final_dropout (可选)

        # 步骤 1: 模拟原始的 act_fn = GELU(...) 模块的行为
        # a. 内部的线性投影
        hidden_states = self.FFN_proj_in(hidden_states)
        # b. 内部的激活函数
        hidden_states = self.activation(hidden_states)

        # 步骤 2: 模拟原始的 self.net[1]
        hidden_states = self.dropout(hidden_states)

        # 步骤 3: 模拟原始的 self.net[2]
        hidden_states = self.FFN_proj_out(hidden_states)

        # 步骤 4: 模拟原始的 self.net[3] (如果存在)
        hidden_states = self.final_dropout(hidden_states)

        return hidden_states


class FeedForward_third(nn.Module):
    r"""
    一个对 PEFT/LoRA 友好的前馈网络层 (FFN) 的重写版本。
    ...
    def __init__(
        ...
        # 1. 暴露第一个线性层 (原先封装在 GELU 模块内部)
        # 这个定义与原始 GELU 类内部的 self.proj 完全一致
        self.th_proj_in = nn.Linear(dim, inner_dim, bias=bias)
        ...
        # 4. 暴露第二个线性层 (原先是 self.net[2])
        self.th_proj_out = nn.Linear(inner_dim, dim_out, bias=bias)
    ...
    def forward(self, hidden_states: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        ...
        # 步骤 1: 模拟原始的 act_fn = GELU(...) 模块的行为
        # a. 内部的线性投影
        hidden_states = self.th_proj_in(hidden_states)
        # b. 内部的激活函数
        hidden_states = self.activation(hidden_states)
        ...
        # 步骤 3: 模拟原始的 self.net[2]
        hidden_states = self.th_proj_out(hidden_states)
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_attention_heads, attention_head_dim, qk_norm="rms_norm", eps=1e-6):
        ...
        self.ff_thirdmodal = FeedForward_third(dim=dim, dim_out=dim, activation_fn="gelu-approximate")
