...
class Trimodal_Attention(nn.Module):
    r"""
    ...
    Parameters:
        ...
        新增的第三个模态的参数由trimodal_kv_proj_dim: Optional[int] = None,trimodal_output: Optional[int] = None来控制，其中trimodal_kv_proj_dim用来控制qkv及其norm是否初始化，trimodal_output
        控制是否在输出前经过线性层。
    """
    ...
    def __init__(
        ...
        trimodal_kv_proj_dim: Optional[int] = None,
        trimodal_output: Optional[int] = None,
        ...
        self.trimodal_kv_proj_dim = trimodal_kv_proj_dim
        self.trimodal_output = trimodal_output
        ...
        # 新增的第三个模态的参数
        if self.trimodal_kv_proj_dim is not None:
            self.third_add_q_proj = nn.Linear(trimodal_kv_proj_dim, self.inner_dim, bias=True)
            self.third_add_k_proj = nn.Linear(trimodal_kv_proj_dim, self.inner_kv_dim, bias=True)
            self.third_add_v_proj = nn.Linear(trimodal_kv_proj_dim, self.inner_kv_dim, bias=True)
        else:
            self.third_add_q_proj = None
            self.third_add_k_proj = None
            self.third_add_v_proj = None
        ...
        #新增的第三个模态的参数
        if self.trimodal_output is not None and not self.context_pre_only:
            self.to_thirdmodal_out = nn.Linear(self.inner_dim, self.out_context_dim, bias=out_bias)
        else:
            self.to_thirdmodal_out = None
        ...
        """与原版的FluxAttnProcessor2_0相比，新添加的模块有：third_add_q_proj， third_add_k_proj， third_add_v_proj，norm_thirdmodal_q，norm_thirdmodal_k，to_thirdmodal_out"""

        # 新增的第三个模态的参数
        if qk_norm is not None and trimodal_kv_proj_dim is not None:
            if qk_norm == "layer_norm":
                self.norm_thirdmodal_q = nn.LayerNorm(dim_head, eps=eps, elementwise_affine=elementwise_affine)
                self.norm_thirdmodal_k = nn.LayerNorm(dim_head, eps=eps, elementwise_affine=elementwise_affine)
            elif qk_norm == "fp32_layer_norm":
                self.norm_thirdmodal_q = FP32LayerNorm(dim_head, elementwise_affine=False, bias=False, eps=eps)
                self.norm_thirdmodal_k = FP32LayerNorm(dim_head, elementwise_affine=False, bias=False, eps=eps)
            elif qk_norm == "rms_norm":
                self.norm_thirdmodal_q = RMSNorm(dim_head, eps=eps)
                self.norm_thirdmodal_k = RMSNorm(dim_head, eps=eps)
            elif qk_norm == "rms_norm_across_heads":
                # Wan applies qk norm across all heads
                # Wan also doesn't apply a q norm
                self.norm_thirdmodal_q = None
                self.norm_thirdmodal_k = RMSNorm(dim_head * kv_heads, eps=eps)
            else:
                raise ValueError(
                    f"unknown qk_norm: {qk_norm}. Should be one of `None,'layer_norm','fp32_layer_norm','rms_norm'`"
                )
    ...
    def forward(
        ...
        thirdmodal_hidden_states: torch.FloatTensor = None,
...
class Trimodal_FluxAttnProcessor2_0:
    """输入是hidden_states（图）， encoder_hidden_states（文）， thirdmodal_hidden_states（脑）， attention_mask，
       注意，在输入的时候是把数据拼接成  文+图+脑
       输出是hidden_states, encoder_hidden_states, thirdmodal_hidden_states
       与原版的FluxAttnProcessor2_0相比，新添加的模块有：third_add_q_proj， third_add_k_proj， third_add_v_proj，norm_thirdmodal_q，norm_thirdmodal_k，to_thirdmodal_out
    """

    def __init__(self):
        if not hasattr(F, "scaled_dot_product_attention"):
            raise ImportError("Trimodal_FluxAttnProcessor2_0 requires PyTorch 2.0, to use it, please upgrade PyTorch to 2.0.")

    def __call__(
        self,
        attn: Trimodal_Attention,
        ...
        if thirdmodal_hidden_states is not None:
            thirdmodal_hidden_states_query_proj = attn.third_add_q_proj(thirdmodal_hidden_states)
            thirdmodal_hidden_states_key_proj = attn.third_add_k_proj(thirdmodal_hidden_states)
            thirdmodal_hidden_states_value_proj = attn.third_add_v_proj(thirdmodal_hidden_states)

            thirdmodal_hidden_states_query_proj = thirdmodal_hidden_states_query_proj.view(
                batch_size, -1, attn.heads, head_dim
            ).transpose(1, 2)
            thirdmodal_hidden_states_key_proj = thirdmodal_hidden_states_key_proj.view(
                batch_size, -1, attn.heads, head_dim
            ).transpose(1, 2)
            thirdmodal_hidden_states_value_proj = thirdmodal_hidden_states_value_proj.view(
                batch_size, -1, attn.heads, head_dim
            ).transpose(1, 2)


            if attn.norm_thirdmodal_q is not None:
                thirdmodal_hidden_states_query_proj = attn.norm_thirdmodal_q(thirdmodal_hidden_states_query_proj)
            if attn.norm_thirdmodal_k is not None:
                thirdmodal_hidden_states_key_proj = attn.norm_thirdmodal_k(thirdmodal_hidden_states_key_proj)

            # attention
            query = torch.cat([query, thirdmodal_hidden_states_query_proj], dim=2)
            key = torch.cat([key, thirdmodal_hidden_states_key_proj], dim=2)
            value = torch.cat([value, thirdmodal_hidden_states_value_proj], dim=2)
        ...
        if thirdmodal_hidden_states is not None:
            # 三模态情况下的切分和返回
            encoder_hidden_states_out = hidden_states[:, :text_seq_len]
            hidden_states_out = hidden_states[:, text_seq_len: text_seq_len + image_seq_len]
            thirdmodal_hidden_states_out = hidden_states[:, text_seq_len + image_seq_len:]
            ...
            if attn.to_thirdmodal_out is not None:
                thirdmodal_hidden_states_out = attn.to_thirdmodal_out(thirdmodal_hidden_states_out)

            return hidden_states_out, encoder_hidden_states_out, thirdmodal_hidden_states_out
