...
# 加载VQVAE模型
vq_model = VQModel.from_pretrained("/nfs/diskstation/DataStation/ChangdeDu/Muddit", subfolder="vqvae")
...
# 创建数据集和数据加载器
root_dir = "/nfs/diskstation/DataStation/public_dataset/NSD_complete/NSD_imgs/"
dataset = ImageCaptionLargeDataset(root_dir, size=512, norm=False)
batch_size = 64  # 根据GPU内存调整批次大小
dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=4)

# 创建输出目录
output_dir = "/nfs/diskstation/DataStation/public_dataset/NSD_complete/NSD_features/VQVAE_feature_img"
...
with torch.no_grad():
    for batch_idx, (batch_images, batch_filenames) in enumerate(tqdm(dataloader, desc="处理图像批次")):
        if batch_idx == 1140:
            ...
            try:
                # 编码图像
                encoded = vq_model.encode(batch_images)

                # 获取VQ编码
                # 注意：这里可能需要根据您的VQVAE模型结构调整索引
                vq_ids = vq_model.quantize(encoded.latents)[2][2].reshape(
                    batch_images.shape[0], height // vae_scale_factor, width // vae_scale_factor
                )

                # 将VQ编码转换为CPU和numpy数组
                vq_ids_np = vq_ids.cpu().numpy()
                ...
                for i in range(len(batch_filenames)):
                    ...
                    # 保存VQ编码
                    np.save(output_path, vq_ids_np[i])
